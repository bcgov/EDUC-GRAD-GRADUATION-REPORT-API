...
public class RESTService {
    ...
    @Autowired
    public RESTService(@Qualifier("graduationReportApiClient") WebClient graduationServiceWebClient) {
        this.webClient = graduationServiceWebClient;
    }
    ...
    public <T> T get(String url, Class<T> clazz, WebClient webClient) {
        ...
        try {
            obj = webClient
                    .get()
                    .uri(url)
                    .headers(h -> h.set(ReportApiConstants.CORRELATION_ID, ThreadLocalStateUtil.getCorrelationID()))
                    .retrieve()
    ...
    public <T> T post(String url, Object body, ParameterizedTypeReference<T> typeRef, WebClient webClient) {
        T obj;
        if (webClient == null)
            webClient = this.webClient;
        try {
    ...
    public <T> List<T> postForList(String url, Object body, Class<T> clazz, WebClient webClient) {
        try {
