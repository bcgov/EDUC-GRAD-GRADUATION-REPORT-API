...
import ca.bc.gov.educ.api.grad.report.model.dto.v2.reports.fetch.GraduationStatus;
...
import ca.bc.gov.educ.api.grad.report.util.StudentAssessmentDuplicatesWrapper;
...
public class ReportService {
    ...
    @Autowired
    public ReportService(JsonTransformer jsonTransformer, ReportApiConstants ReportApiConstants,
        ...
        this.ReportApiConstants = ReportApiConstants;
    ...
    public ProgramCertificateTranscript getTranscript(GraduationStudentRecord gradResponse,
                                                      ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus,
        ...
        try {
            return codeService.getProgramTranscript(req);
        } catch (Exception e) {
    ...
    private UUID getSchoolOfRecordId(GraduationStudentRecord gradResponse, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus) {
    ...
    public String getSchoolCategoryCode(UUID schoolId) {
        ...
        SchoolClob school = this.restService.get(String.format(ReportApiConstants.getSchoolClobBySchoolIdUrl(), schoolId),
                SchoolClob.class, graduationApiClient);
        return (school == null) ? null : school.getSchoolCategoryLegacyCode();
    }

    public ReportData prepareTranscriptData(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus,
    ...
    public ReportData prepareTranscriptData(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, boolean xml, ExceptionMessage exception) {
        try {
    ...
    public ReportData prepareTranscriptData(String pen, boolean xml, ExceptionMessage exception) {
        try {
            Pair<GraduationStudentRecord, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData> graduationStudentRecord = getGraduationStudentRecordAndGraduationData(pen);
    ...
    private GradSearchStudent getStudentByPenFromStudentApi(String pen) {
        var response = restService.get(String.format(ReportApiConstants.getPenStudentApiByPenUrl(), pen),
    ...
    private GradSearchStudent getStudentByStudentIDFromStudentApi(UUID studentID) {
        return restService.get(String.format(ReportApiConstants.getPenStudentApiByIDUrl(), studentID),
    ...
    private GraduationStudentRecord getGradStatusFromGradStudentApi(String studentID) {
        GraduationStudentRecord graduationStudentRecord = restService
                .get(String.format(ReportApiConstants.getReadGradStudentRecord(), studentID),
    ...
    @Generated
    private List<NonGradReason> getNonGradReasons(String gradProgramCode, List<ca.bc.gov.educ.api.grad.report.model.dto.GradRequirement> nonGradReasons,
        ...
        if (nonGradReasons != null) {
            ...
            for (ca.bc.gov.educ.api.grad.report.model.dto.GradRequirement gR : nonGradReasons) {
    ...
    @Generated
    private Transcript getTranscriptData(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, GraduationStudentRecord gradResponse, boolean xml, ExceptionMessage exception) {
    ...
    @Generated
    private void createCourseListForTranscript(List<StudentCourse> studentCourseList, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, List<TranscriptResult> tList, String provincially, boolean xml) {
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("PST"), Locale.CANADA);
        String today = EducGraduationApiUtils.formatDate(cal.getTime(), ReportApiConstants.DEFAULT_DATE_FORMAT);
        for (StudentCourse sc : studentCourseList) {
            Date sessionDate = toLastDayOfMonth(EducGraduationApiUtils.parseDate(sc.getSessionDate() + "/01", ReportApiConstants.SECONDARY_DATE_FORMAT));
            String sDate = EducGraduationApiUtils.formatDate(sessionDate, ReportApiConstants.DEFAULT_DATE_FORMAT);
    ...
    private TranscriptCourse setCourseObjForTranscript(StudentCourse sc, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus) {
    ...
    private String checkCutOffCourseDate(String sDate, Double value) {
        ...
        Date temp = toLastDayOfMonth(EducGraduationApiUtils.parseDate(sessionDate, ReportApiConstants.SECONDARY_DATE_FORMAT));
        sessionDate = EducGraduationApiUtils.formatDate(temp, ReportApiConstants.DEFAULT_DATE_FORMAT);
    ...
    @Generated
    private void createAssessmentListForTranscript(List<StudentAssessment> studentAssessmentList, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, List<TranscriptResult> tList, boolean xml) {
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("PST"), Locale.CANADA);
        String today = EducGraduationApiUtils.formatDate(cal.getTime(), ReportApiConstants.DEFAULT_DATE_FORMAT);
        ...
        for (StudentAssessment sc : processList) {
            ...
            if (!sc.isDuplicate() && !sc.isFailed() && !sc.isNotCompleted() && ((notCompletedCourse) || !sc.isProjected())) {
                ...
                if (!skipProcessing) {
                    ...
                    String cutoffDate = EducGraduationApiUtils.formatDate(graduationDataStatus.getGradProgram().getAssessmentReleaseDate(), ReportApiConstants.DEFAULT_DATE_FORMAT);
                    if(sc.getSessionDate() != null) {
    ...
    @Generated
    private List<TranscriptResult> getTranscriptResults(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, boolean xml) {
    ...
    private String getSpecialCase(StudentAssessment sA) {
        String finalPercent;
        SpecialCase spC = restService.get(String.format(ReportApiConstants.getSpecialCase(), sA.getSpecialCase()),
    ...
    private GraduationData getGraduationData(
            ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, GraduationStudentRecord graduationStudentRecord) {
    ...
    private GradProgram getGradProgram(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus) {
        ...
        if (graduationDataStatus.getGradStatus().getProgram() != null) {
            GraduationProgramCode gradProgram = restService
                    .get(String.format(ReportApiConstants.getProgramNameEndpoint(), graduationDataStatus.getGradStatus().getProgram()),
    ...
    private List<ProgramRequirementCode> getAllProgramRequirementCodeList() {
        var response = this.restService.get(ReportApiConstants.getProgramRequirementsEndpoint(), List.class, graduationApiClient);
        return jsonTransformer.convertValue(response, new TypeReference<List<ProgramRequirementCode>>(){});
    }

    private Student getStudentData(ca.bc.gov.educ.api.grad.report.model.dto.GradSearchStudent gradStudent, GraduationStudentRecord gradResponse) {
    ...
    private School getSchoolAtGradData(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus) {
    ...
    private GraduationStatus getGraduationStatus(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationData, School schoolAtGrad, School schoolOfRecord) {
    ...
    private Student getStudentDataAchvReport(ca.bc.gov.educ.api.grad.report.model.dto.GradSearchStudent studentObj, List<StudentOptionalProgram> optionalStudentProgram) {
    ...
    private void getStudentCoursesAssessmentsNExams(ReportData data, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus) {
    ...
    public ReportData prepareCertificateData(ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, ExceptionMessage exception) {
        try {
            String studentID = graduationDataStatus.getGradStudent().getStudentID();
            if (studentID == null) {
                throw new EntityNotFoundException(
    ...
    public ReportData prepareCertificateData(GraduationStudentRecord gradResponse,
                                             ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, ProgramCertificateTranscript certType) {
        ReportData data = new ReportData();
        GraduationData graduationData = getGraduationData(graduationDataStatus, gradResponse);
    ...
    public ReportData prepareAchievementReportData(UUID studentID, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationDataStatus, List<StudentOptionalProgram> optionalProgramList, ExceptionMessage exception) {
        try {
            School schoolAtGrad = getSchoolAtGradData(graduationDataStatus);
            School schoolOfRecord = getSchoolData(graduationDataStatus.getSchool());
            ReportData data = new ReportData();
    ...
    public Pair<GraduationStudentRecord, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData> getGraduationStudentRecordAndGraduationData(String pen) {
    ...
    private void overrideDemogProperties(GradSearchStudent student, ca.bc.gov.educ.api.grad.report.model.dto.GraduationData graduationData) {
    ...
    private List<GradRequirement> getRequirementsMetAchvReport(String gradProgramCode, List<ca.bc.gov.educ.api.grad.report.model.dto.GradRequirement> optionalRequirementsMet, StudentCourses optionalStudentCourses, List<NonGradReason> nonGradReasons) {
        List<GradRequirement> grList = new ArrayList<>();
        for (ca.bc.gov.educ.api.grad.report.model.dto.GradRequirement gr : optionalRequirementsMet) {
            if (!gr.isProjected()) {
    ...
    private boolean isGradDatePast(String programCompletionDate) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat(programCompletionDate.length() < 10? ReportApiConstants.SECONDARY_DATE_FORMAT : ReportApiConstants.DEFAULT_DATE_FORMAT);
        try {
