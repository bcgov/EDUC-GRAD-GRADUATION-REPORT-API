...
public class StudentTranscriptService extends GradReportService{
    ...
    @Autowired
    public StudentTranscriptService(ProgramCertificateTranscriptRepository programCertificateTranscriptRepository, StudentTranscriptRepository studentTranscriptRepository) {
    ...
    private StudentTranscriptReport createTranscriptReport(final ReportFormat format, final boolean preview) throws IOException {
    ...
    private Transcript adapt(final TranscriptTypeCode transcriptTypeCode, final GraduationProgramCode graduationProgramCode, final List<TranscriptCourse> transcriptCourses, final Date issueDate, final boolean interim) {
    ...
    private List<TranscriptResult> adapt(
        ...
        if (transcriptCourses != null) {
            ...
            for (final TranscriptCourse transcriptCourse : transcriptCourses) {
                ...
                final TranscriptResult tResult = new TranscriptResult(
                        req, eq, ufg, reqName);
    ...
    private synchronized StudentTranscriptReport createReport(
        ...
        StudentImpl stu = (StudentImpl)report.getDataSource();
    ...
    private GraduationData adaptGraduationData(
            ...
            final Transcript transcript) {
        final GraduationData graduationData = new GraduationData();

        LocalDate gradDate = studentInfo.getGradDate();

        graduationData.setGraduationDate(gradDate);
        graduationData.setHonorsFlag(studentInfo.isHonourFlag());
        graduationData.setDogwoodFlag(studentInfo.isDogwoodFlag());
        graduationData.setProgramNames(studentInfo.getAcademicProgram());
        final String creditsUsedForGrad = getCreditsUsedForGrad(transcript);
        graduationData.setTotalCreditsUsedForGrad(creditsUsedForGrad);
